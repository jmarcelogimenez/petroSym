...
## Evitar que haya decisiones en el generateItems, por lo cual es necesarios
## modificar la estructura que genera las combobox
...
dicPCG = {}
dicPCG['solver'] = ['PCG', 3]
dicPCG[0] = ['preconditioner', 'Preconditioner: ', 2, 'cb_pre_pcg', 4,
['DIC', 'FDIC', 'diagonal', 'none'], 1]
dicPCG[1] = ['tolerance', 'Tolerance: ', 0, 'line_tolerance_pcg',
"-?\d+\.?\d*e-?\d+", 1e-6]
dicPCG[2] = ['relTol', 'Relative Tolerance: ', 0, 'line_reltolerance_pcg',
"-?\d+\.?\d*e-?\d+", 0]

dicPBiCG = {}
dicPBiCG['solver'] = ['PBiCG', 3]
dicPBiCG[0] = ['preconditioner', 'Preconditioner: ', 2, 'cb_pre_pbicg', 3,
['DILU', 'diagonal', 'none'], 1]
dicPBiCG[1] = ['tolerance', 'Tolerance: ', 0, 'line_tolerance_pbicg',
"-?\d+\.?\d*e-?\d+", 1e-6]
dicPBiCG[2] = ['relTol', 'Relative Tolerance: ', 0, 'line_reltolerance_pbicg',
"-?\d+\.?\d*e-?\d+", 0]

dicGAMG = {}
dicGAMG['solver'] = ['GAMG', 9]
dicGAMG[0] = ['smoother', 'Smoother: ', 2, 'cb_smoother_gamg', 3,
['GaussSeidel', 'DIC', 'DICGaussSeidel']]
dicGAMG[1] = ['cacheAgglomeration', 'Cache Agglomeration: ', 3,
'chb_agg_gamg'] #3 checkbox
dicGAMG[2] = ['agglomerator', 'Agglomerator: ', 2, 'cb_agg_gamg', 1,
['faceAreaPair'], 1]
dicGAMG[3] = ['nCellsInCoarsestLevel', 'Cells In Coarsest Level: ', 1,
'spin_cicl_gamg']
...
dicGAMG[7] = ['tolerance', 'Tolerance: ', 0, 'line_tolerance_gamg',
"-?\d+\.?\d*e-?\d+", 1e-6]
dicGAMG[8] = ['relTol', 'Relative Tolerance: ', 0, 'line_reltolerance_gamg',
"-?\d+\.?\d*e-?\d+", 0]

dicSmoothSolver = {}
dicSmoothSolver['solver'] = ['smoothSolver', 4]
dicSmoothSolver[0] = ['smoother', 'Smoother: ', 2, 'cb_smoother_ss', 3,
['GaussSeidel', 'DIC', 'DICGaussSeidel']] #2 combobox
dicSmoothSolver[1] = ['nSweeps', 'Sweeps: ', 1, 'spin_Sweeps_ss'] #1 spinbox
dicSmoothSolver[2] = ['tolerance', 'Tolerance: ', 0, 'line_tolerance_ss',
 "-?\d+\.?\d*e-?\d+", 1e-6] #0 lineedit
dicSmoothSolver[3] = ['relTol', 'Relative Tolerance: ', 0, 'line_reltol_ss',
...
class solverSettings(solverSettingsUI):
    ...
    def addTabs(self, ipatch=None):
        ...
        #Dict que guarda el indice del solver actual
        self.CurrentWidgetIndexDict = dict()
        #Dict que guarda el indice del solver original
        self.OriginalWidgetIndexDict = dict()
        ...
        for ifield in self.fields:
            ...
            #Por default, si no existe el campo en el documento, que empiece
            #con smoothsolver (Porque es comun para simetricos y no simetricos)
            if not self.parsedData.content['solvers'].has_key(ifield):
                templateparsedData = deepcopy(self.templateparsedDataI)
                self.parsedData.content['solvers'][ifield] = \
                templateparsedData['solvers']['smoothSolver']

            #Alguna manera mas limpia de hacer esto?
            if (self.parsedData['solvers'][ifield]['solver'] == 'PBiCG') or \
            (self.parsedData['solvers'][ifield]['solver'] == 'PCG'):
            ...
            #Parche para que el cambio de ventanas no pierda los datos
            self.OriginalWidgetIndexDict[cb.objectName()] = cb.currentIndex()
            self.modifyTables(self.tabWidget.widget(self.tabWidget.count()-1)
            , True)
            self.tabWidget.setTabText(self.tabWidget.count(), ifield)

            QtCore.QObject.connect(cb, QtCore.SIGNAL(_fromUtf8
            ("currentIndexChanged(int)")), self.onChangeTables)
        ...
        if self.solvername in dic_solversPIMPLE:
            ...
            i = 0
             #Cargo todos los que haya en el dic
            for ifield in self.fieldsoriginal:
                dicRelaxation[i] = [ifield, ifield+": ", 0, "line_"+ifield+
                "_relax", "0\.\d+|1|0\.\d+e-\d+|1e-\d+", 1]
                i = i+1
            ...
            #Para que no patee setValues
            self.OriginalWidgetIndexDict['Relaxation'] = -1
    ...
    def loadData(self):
        ...
        for ikey in self.parsedData['solvers'].keys():
            ...
            if ikey[0]=='"':
                ...
                ikey = ikey.replace(')','')
                ikey = ikey.replace('(','')
                #Si en esos fields esta la palabra final
                if ikey.find('Final',0,len(ikey)) > -1:
                    final = True
    ...
    def modifyTables(self, widget, creation):
        ...
        #Boleano que me dice si cambio de solver
        noCreado = creation or \
        (self.CurrentWidgetIndexDict[cb.objectName()] != cb.currentIndex())
        ...
        if currentIndex == 0:
            ...
            if solver in dic_unknowsSymetric:
                ...
                self.setValues(creation, dicPCG, solver, widget,
                               currentIndex)
            else:
                for ifield in range(0, dicPBiCG['solver'][1]):
                    self.generateItems(dicPBiCG[ifield], layout, ifield)

                self.setValues(creation, dicPBiCG, solver, widget,
                               currentIndex)

        elif currentIndex == 1:

            #self.clearLayout(layout,1)  #Limpio y creo de nuevo
            self.changeValuesOnCB(0, [['GaussSeidel', 'DIC',
            'DICGaussSeidel'], ['GaussSeidel']], dicGAMG,
            (solver in dic_unknowsSymetric))
    ...
    def onChangeTables(self):
         #Se modifica solo donde estoy parado
        self.modifyTables(self.tabWidget.currentWidget(), False)
    ...
    def aplicar(self):
        ...
        for itab in range(self.tabWidget.count()):
            ...
            #Esta es la cb del solver
            cb = widget.findChildren(QtGui.QComboBox)[0]
            ...
            if currentIndex == 0:
                ...
                if solver in dic_unknowsSymetric:
                    self.parsedData['solvers'][str(solver)] = \
                    templateparsedData['solvers']['PCG']
                    for ifield in range(0, dicPCG['solver'][1]):
                        self.saveItems(widget, dicPCG, ifield,
                                      self.parsedData['solvers'][str(solver)])
                else:
                    self.parsedData['solvers'][str(solver)] = \
                    templateparsedData['solvers']['PBiCG']
                    for ifield in range(0, dicPBiCG['solver'][1]):
                        self.saveItems(widget, dicPBiCG, ifield,
                                      self.parsedData['solvers'][str(solver)])

            elif currentIndex == 1:
                self.parsedData['solvers'][str(solver)] = \
                templateparsedData['solvers']['GAMG']
                ...
                for ifield in range(0, dicGAMG['solver'][1]):
                    self.saveItems(widget, dicGAMG, ifield,
                                   self.parsedData['solvers'][str(solver)])
                ...
                for ifield in range(0, dicSmoothSolver['solver'][1]):
                    self.saveItems(widget, dicSmoothSolver, ifield,
                                   self.parsedData['solvers'][str(solver)])
        ...
        if self.solvername in dic_solversPIMPLE:
            ...
            for ifield in range(0, dicPIMPLE['solver'][1]):
                self.saveItems(widget, dicPIMPLE, ifield,
                               self.parsedData['PIMPLE'])

            #Relaxation
            widget = self.tabWidget.findChild(QtGui.QWidget, 'Relaxation')
            templateparsedData = deepcopy(self.templateparsedDataI)
            self.parsedData['relaxationFactors'] = \
            templateparsedData['solvers']['relaxationFactors']
            self.parsedData['relaxationFactors']['fields'] = \
            templateparsedData['solvers']['relaxationFactors']['fields']
            for ifield in range(0, dicRelaxation['solver'][1]):
                self.saveItems(widget, dicRelaxation, ifield,
                               self.parsedData['relaxationFactors']['fields'])
    ...
    def generateItems(self, dic, layoutV, ifield):
        ...
        if tipo == 0:
            ...
            QtCore.QObject.connect(line, QtCore.SIGNAL(
            _fromUtf8("textChanged(QString)")), self.onChangeSomething)
        elif tipo == 1:
            spin = QtGui.QSpinBox()
            spin.setObjectName(dic[3])
            spin.setFixedWidth(120)
            layoutH.addWidget(spin)
            QtCore.QObject.connect(spin, QtCore.SIGNAL(
            _fromUtf8("valueChanged(int)")), self.onChangeSomething)
        elif tipo == 2:
            cb = QtGui.QComboBox()
            ...
            layoutH.addWidget(cb)
            QtCore.QObject.connect(cb, QtCore.SIGNAL(
            _fromUtf8("currentIndexChanged(int)")), self.onChangeSomething)
        elif tipo == 3:
    ...
    #(Solo para fields)
        ...
        for ifield in range(0, dic['solver'][1]):
            ...
            #Si hay alguna que falta, no importa ?
            if dic[ifield][0] not in parsedData.keys():
                continue
